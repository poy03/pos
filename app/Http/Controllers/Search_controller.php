  ...
  public function items(Request $request, $arg='')
  {
    ...
    if($arg=="sales"){
      ...
      if($request->search_by == "category"){
        $search_results = $items->select('quantity', 'category', 'itemname', 'itemID');
        $search_results->where('deleted',0);
        $search_results->where('category', 'like', '%'.$q.'%');
      }else{
      ...
      if($request->search_by == "category"){
        foreach ($search_results as $result) {
          $category = ' ['.htmlspecialchars_decode($result->category).'] ';
          $data[] = array("label"=>$category.htmlspecialchars_decode($result->itemname),"data"=>$result->itemID);
        }
      }else{
        foreach ($search_results as $result) {
